...
router.post('/addnewrestaurant',upload.any(), function(req, res, next) {
    ...
    pool.query("insert into restaurant(restaurant_name, owner_name, address, state, city, zipcode, location, emailaddress, type, mobilenumber, idproof, idproofimage, shopact, shopactimage, fssai, fssaiimage, gst, status, logo, password) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",[req.body.restaurant_name,req.body.owner_name,req.body.address,req.body.state,req.body.city,req.body.zipcode,req.body.location,req.body.emailaddress,req.body.type,req.body.mobilenumber,req.body.id,req.files[0].originalname,req.body.shopact,req.files[1].originalname,req.body.fssai,req.files[2].originalname,req.body.gst,req.body.status,req.files[3].originalname,req.body.password],function(error,result){
...
router.post('/updateShopActImage',upload.single('shopactimage'),function(req,res){
 pool.query("update restaurant set shopactimage=? where restaurant_id=?",[req.file.originalname,req.body.restaurantid],function(error,result){
 ...
 router.post('/deleterestaurant',function(req,res){
    pool.query("delete from restaurant where restaurant_id=?",[req.body.restaurantid],function(error,result){
