...
export default function Home(props) {
  ...
  const fetchFoodItems = async (typeid) => {
    ...
    var list = await postData("allfooditems/listallfooditemsbyfoodtype", body);
    setFoodItemsList(list);
  };

  const fetchFoodItemsByOffer = async (resid) => {
    var body = {restaurant_id:resid };
    var list = await postData("allfooditems/listfooditemsoffer", body);
    setFoodItemsList(list);
  };
    ...
    return foodItemsList.map((item, index) => {
      return (
        ...
        >
          ...
          {item.fooditemtype="Veg"?<img src='/veg.jpg' width="10" style={{position:'absolute',zIndex:1,padding:10}}/>:<img src='/nonveg.png' width="10" style={{position:'absolute',zIndex:1,padding:10}}/>}
            <img
  ...
  const searchFood=async(value)=>{
    ...
    var list =await postData('allfooditems/searchfooditems',body)
    setFoodItemsList(list)
  ...
  return (
    <div>
      <Header history={props.history}    searchFood={searchFood}  />
