...
router.post('/generateorder', function(req, res, next) {
  pool.query("insert into ordergeneration (orderdate,ordertime,mobileno,emailid,totalamount,orderstatus)values(?,?,?,?,?,?)",[req.body.orderdate,req.body.ordertime,req.body.mobileno,req.body.emailid,req.body.totalamt,req.body.orderstatus],function(error,result){
   if(error)
   {  console.log(error)
...
router.post('/submitorder', function(req, res, next) {
 ...
 router.get("/listordergeneration",function(req,res){
    pool.query("select * from ordergeneration",function(error ,result){
    if(error)
    {
        res.status(500).json([])
    }    
...
router.post("/listorders",function(req,res){
